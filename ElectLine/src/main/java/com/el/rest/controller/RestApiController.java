...
public class RestApiController {
	...
	/*@RequestMapping(value="/survee_data/surveyor/{email}", method=RequestMethod.GET)
	public Iterable<SurveyMain> getVoterBySurveyor(@PathVariable(name="email") String email)
	{
		Iterable<SurveyMain> data =  mainImp.findAllByAssign(assignSurv.findByAssignId(surveyorImp.findBySurvEmail(email).getAssignID()));
		return data;
	}*/
	...
	private List<SurvData> rawData(List<SurveyMain> data)
	{
		...
		for(SurveyMain sm: data)
		{
			...
			for(SurveeFamily sf: sm.getSurveeFamily())
			{
				...
				.append(sf.getSurveeFamilyMemberContact()).append(": ")
				.append(sf.getSurveeFamilyMemberAge()).append(": ")
				.append(String.valueOf(sf.isSurveeFamilyMemberEmployed())).append(": ")
			...
			{
				surveeRelative.append(sr.getSurveeRelativeName()).append(": ")
				.append(sr.getSurveeRelativeContact()).append(": ")
				.append(sr.getSurveeRelativeAddress()).append(" /");
			}
			...
			{
				dynQues.append(dq.getDQues().getDyQuesn()).append("? : ")
				.append(dq.getDAns()).append("  _/ ");
			}
	...
	public void getVoterBySurveyor(@PathVariable(name="email") String email, HttpServletResponse response) throws IOException
	{
		List<SurveyMain> data =  mainImp.findAllByAssign(assignSurv.findByAssignId(surveyorImp.findBySurvEmail(email).getAssignID()));
		//return data;
	...
	public void getVoterByVidhansabha(@PathVariable(name="id") Integer id, HttpServletResponse response) throws IOException
	{
				...
				if(srv.isActiveSurveyor()) {
					...
					if(dt== null)
					{
						...
						if(dt.getBoothData().isEmpty() || dt.getGramPanchayatName().isEmpty())
						{
							...
							return "{\"Login\":\"Success\",\"assign\":"+mapper.writeValueAsString(dt)+",\"status\":2004}";
						}
					}
				}
				else
